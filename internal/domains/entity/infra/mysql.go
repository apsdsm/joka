...
func (m *MySQLDBAdapter) UpdateRow(ctx context.Context, table, pkColumn string, pkValue int64, columns map[string]any) error {
...
// pkValue. Byte-slice values (MySQL's representation of text columns) are
// converted to strings so callers can compare them against resolved values.
func (m *MySQLDBAdapter) GetRow(ctx context.Context, table string, columns []string, pkColumn string, pkValue int64) (map[string]any, error) {
	...
	err := m.db.QueryRowContext(ctx, query, pkValue).Scan(scan...)
	if err == sql.ErrNoRows {
