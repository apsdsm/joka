...
func TestParseEntityAction(t *testing.T) {
	t.Run("it parses a basic entity with table, ref ID, and columns", func(t *testing.T) {
		dir := t.TempDir()
...
`
	...
	t.Run("it parses parent-child relationships via _has", func(t *testing.T) {
		dir := t.TempDir()
		path := filepath.Join(dir, "test.yaml")
...
`
	...
	t.Run("it parses multiple top-level entities", func(t *testing.T) {
		dir := t.TempDir()
		path := filepath.Join(dir, "test.yaml")
...
`
	...
	t.Run("it returns an error when _is is missing", func(t *testing.T) {
		dir := t.TempDir()
		path := filepath.Join(dir, "test.yaml")
...
`
	...
	t.Run("it leaves refID empty when _id is absent", func(t *testing.T) {
		dir := t.TempDir()
		path := filepath.Join(dir, "test.yaml")
...
`
	...
	t.Run("it preserves non-string values like integers and booleans", func(t *testing.T) {
		dir := t.TempDir()
		path := filepath.Join(dir, "test.yaml")
