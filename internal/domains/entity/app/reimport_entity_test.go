...
func TestReimportEntityAction_Success(t *testing.T) {
	dir := t.TempDir()
...
`
	...
	// Verify deletions happened in reverse order (profiles first, then users).
	...
	// Verify tracking rows were cleared.
	...
	// Verify re-insert happened (parent + child).
...
func TestReimportEntityAction_NotSynced(t *testing.T) {
	db := newMockDBAdapter()
...
func TestReimportEntityAction_DeleteRowFKError(t *testing.T) {
	dir := t.TempDir()
...
func TestReimportEntityAction_NoTrackedRows(t *testing.T) {
	dir := t.TempDir()
	yamlContent := `entities:
  - _is: users
    name: Admin
`
	...
	// Should still re-insert from YAML.
