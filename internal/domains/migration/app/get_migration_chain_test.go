...
func TestGetMigrationChain(t *testing.T) {
	t.Run("it marks all migrations as applied when all are tracked", func(t *testing.T) {
		dir := t.TempDir()
	...
	t.Run("it marks all migrations as pending when none are tracked", func(t *testing.T) {
		dir := t.TempDir()
		createTestFile(t, dir, "240101000000_first.sql")
		createTestFile(t, dir, "240102000000_second.sql")
	...
	t.Run("it marks applied and pending in a mixed chain", func(t *testing.T) {
		dir := t.TempDir()
		createTestFile(t, dir, "240101000000_first.sql")
		createTestFile(t, dir, "240102000000_second.sql")
	...
	t.Run("it returns an error for a broken chain", func(t *testing.T) {
		dir := t.TempDir()
		createTestFile(t, dir, "240101000000_first.sql")
		createTestFile(t, dir, "240102000000_second.sql")
	...
	t.Run("it returns an error when a tracked migration file is missing", func(t *testing.T) {
		dir := t.TempDir()
		adapter := &mockDBAdapter{
			hasMigrationsTable: true,
			appliedMigrations: []models.MigrationRow{
	...
	t.Run("it returns an empty chain when no files exist", func(t *testing.T) {
		dir := t.TempDir()
		adapter := &mockDBAdapter{
			hasMigrationsTable: true,
			appliedMigrations:  nil,
