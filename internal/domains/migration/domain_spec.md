...
| Column | Type | Notes |
|--------|------|-------|
| `id` | `SERIAL PK` | Insertion order, used to maintain the chain |
...
| Column | Type | Notes |
|--------|------|-------|
| `id` | `SERIAL PK` | Insertion order |
| `migration_index` | `VARCHAR(255) UNIQUE` | Links to the migration that produced this snapshot |
| `schema_snapshot` | `TEXT` | JSON object: `{"table_name": "CREATE TABLE ..."}` for all non-joka tables |
...
1. **Execute SQL** — Read the `.sql` file and run it against the database. PostgreSQL handles
   multiple statements natively; `db.SplitSQLStatements` still splits the file so each statement can
   be applied and reported individually.
2. **Record** — Insert a row into `joka_migrations` with the migration's index.
3. **Snapshot** — Reconstruct a `CREATE TABLE` statement from `pg_catalog` for every non-joka base
   table (`reconstructCreateTable`) and store the set as JSON in `joka_snapshots`. It is rebuilt
   rather than dumped because `information_schema.columns` reports `ARRAY` / `USER-DEFINED` instead
   of real types and cannot express identity.
...
- `ErrDumpToolMissing`, `ErrDumpNotApplicable`, `ErrNotLastApplied` — Consolidation.
...
- `VerifySchemaAction` — Compares the live schema against the stored snapshot; returns `VerifyResult`.
